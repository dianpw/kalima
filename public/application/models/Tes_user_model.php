...
class Tes_user_model extends CI_Model{
    ...
    function update_menit($id_tes_siswa, $waktu){
        $sql = 'UPDATE tes_siswa SET tes_siswa.date_created=TIMESTAMPADD(MINUTE, '.$waktu.', tes_siswa.date_created) WHERE id_tes_siswa="'.$id_tes_siswa.'"';
        $this->db->query($sql);
    }
    ...
    function count_by_status_waktu($id_tes_siswa){
        $this->db->select('COUNT(id_tes_siswa) AS hasil')
                 ->where('(id_tes_siswa="'.$id_tes_siswa.'" AND status="1" AND TIMESTAMPADD(MINUTE, duration_time, tes_siswa.date_created)>NOW())')
                 ->from($this->table)
    ...
    function count_by_status_waktuuser($id_tes_siswa, $waktuuser){
        $this->db->select('COUNT(id_tes_siswa) AS hasil')
                 ->where('(id_tes_siswa="'.$id_tes_siswa.'" AND tes_siswa.status="1" AND TIMESTAMPADD(MINUTE, duration_time, tes_siswa.date_created)>"'.$waktuuser.'")')
                 ->from($this->table)
                 ->join('tes', 'tes_siswa.id_tes = tes.id_tes');
        return $this->db->get();
    }
    ...
    function get_by_user_status($id_siswa){
        $this->db->where('tes_siswa.id_siswa="'.$id_siswa.'" AND tes_siswa.status!=4')
                 ->from($this->table)
                 ->join('tes', 'tes_siswa.id_tes = tes.id_tes');
        return $this->db->get();
    }
    ...
    function get_by_user_tes_limit($id_siswa, $id_tes){
        $this->db->where('tes_siswa.id_siswa="'.$id_siswa.'" AND tes_siswa.id_tes="'.$id_tes.'" AND tes_siswa.status=1')
                 ->from($this->table)
                 ->join('tes', 'tes_siswa.id_tes = tes.id_tes')
                 ->limit(1);
        return $this->db->get();
    ...
    function get_by_group(){
        $this->db->from($this->table)
                 ->join('tes', 'tes_siswa.id_tes = tes.id_tes')
                 ->order_by('tes_siswa.id_tes', 'DESC')
                 ->group_by('tes_siswa.id_tes');
        return $this->db->get();
    }
    ...
    function get_by_tes_group_urut_tanggal($id_tes, $id_kelas, $urutkan, $tanggal, $keterangan){
        $sql = 'tes_siswa.date_created>="'.$tanggal[0].'" AND tes_siswa.date_created<="'.$tanggal[1].'"';
        ...
        if($urutkan=='tertinggi'){
            ...
            $order = 'tes_siswa.date_created DESC';
        }else{
            $order = 'tes_siswa.id_tes ASC';
        }
		
		if(!empty($keterangan)){
			$sql = $sql.' AND siswa.user_detail LIKE "%'.$keterangan.'%"';
		}
	...
	function get_by_tes_group($id_tes, $id_kelas){
        $sql = 'tes_siswa.id_tes="'.$id_tes.'" AND tes_siswa.id_kelas="'.$id_kelas.'"';
    ...
    function get_nilai_by_tes_user($id_tes, $id_siswa){
        $this->db->select('SUM(tes_soal.nilai) AS nilai')
                 ->where('(tes_siswa.id_tes="'.$id_tes.'" AND tes_siswa.id_siswa="'.$id_siswa.'")')
                 ->from($this->table)
	...
	*/
	function get_datatable($start, $rows, $id_tes, $id_kelas, $urutkan, $tanggal, $keterangan, $search){
        $sql = 'tes_siswa.date_created>="'.$tanggal[0].'" AND tes_siswa.date_created<="'.$tanggal[1].'" AND siswa.nama LIKE "%'.$search.'%"';
		
        if($id_tes!='semua'){
            $sql = $sql.' AND tes_siswa.id_tes="'.$id_tes.'"';
        }
        if($id_kelas!='semua'){
            $sql = $sql.' AND siswa.id_kelas="'.$id_kelas.'"';
        }
        $order = '';
        if($urutkan=='tertinggi'){
            $order = 'nilai DESC';
    ...
    function get_datatable_evaluasi($start, $rows, $id_tes, $urutkan){
        $sql = '';
        if(!empty($id_tes)){
            $sql = ' AND tes_siswa.id_tes="'.$id_tes.'"';
        }
        $order = '';
        if($urutkan=='soal'){
            $order = 'tes_soal.id_soal ASC';
        }else{
            $order = 'tes_soal.id_tes_siswa ASC';
        }

        $this->db->select('tes_soal.id_tes_soal, tes_soal.jawaban_text, tes.*, soal.*')
                 ->where('(soal.tipe="2" AND jawaban_text IS NOT NULL AND comment IS NULL '.$sql.' )')
                 ->from($this->table)
                 ->join('tes', 'tes_siswa.id_tes = tes.id_tes')
                 ->join('tes_soal', 'tes_soal.id_tes_siswa = tes_siswa.id_tes_siswa')
    ...
    function get_datatable_operator($start, $rows, $token){
        $this->db->select('tes_siswa.*,kelas.kelas, tes.*, tes.nama AS nama_tes, siswa.*, SUM(tes_soal.nilai) AS nilai ')
                 ->where('(tes_siswa.token IN ('.$token.'))')
                 ->from($this->table)
                 ->join('siswa', 'tes_siswa.id_siswa = siswa.id_siswa')
                 ->join('kelas', 'siswa.id_kelas = kelas.id_kelas')
                 ->join('tes', 'tes_siswa.id_tes = tes.id_tes')
                 ->join('tes_soal', 'tes_soal.id_tes_siswa = tes_siswa.id_tes_siswa')
                 ->group_by('tes_siswa.id_tes_siswa')
                 ->order_by('tes_siswa.date_created DESC')
                 ->limit($rows, $start);
        return $this->db->get();
    }

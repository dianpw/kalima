...
class Jawaban_model extends CI_Model{
    ...
    function get_by_soal_limit($soal, $limit){
        $sql = '(SELECT id_jawaban FROM jawaban WHERE jawaban.id_soal="'.$soal.'" AND jawaban.benar=1 LIMIT 1) UNION (SELECT id_jawaban FROM jawaban WHERE jawaban.id_soal="'.$soal.'" AND jawaban.benar=0 LIMIT '.($limit-1).') ORDER BY RAND();';
	...
	function get_datatable($start, $rows, $kolom, $isi, $soal){
		$this->db->where('('.$kolom.' LIKE "%'.$isi.'%" AND id_soal="'.$soal.'")')
                 ->from($this->table)
				 ->order_by('id_jawaban', 'ASC')
                 ->limit($rows, $start);
        return $this->db->get();
    ...
    function get_datatable_count($kolom, $isi, $soal){
		$this->db->select('COUNT(*) AS hasil')
                 ->where('('.$kolom.' LIKE "%'.$isi.'%" AND id_soal="'.$soal.'")')
                 ->from($this->table);
        return $this->db->get();
	}
}
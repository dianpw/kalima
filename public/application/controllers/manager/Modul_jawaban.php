        	...
        	if($query_soal->num_rows()>0){
        		...
        		if($query_soal->soal_tipe!=2){
					...
					if(!empty($query_soal->soal_audio)){
						$posisi = $this->config->item('upload_path').'/mapel_'.$query_soal->soal_topik_id;
						$soal = $soal.'
							<audio controls>
		...
		if(!empty($id)){
			...
			if($query->num_rows()>0){
				...
				if(!empty($query->soal_audio)){
					$posisi = $this->config->item('upload_path').'/mapel_'.$query->soal_topik_id;
					$soal = $soal.'
						<audio controls>
    ...
    function get_datatable(){
	    ...
	    foreach ($query as $temp) {			
			...
			if($temp->jawaban_benar==1){
				$jawaban = '<div class="alert alert-success d-flex align-items-center" role="alert" style="padding: 5px;margin-bottom: 0;">'.$temp->jawaban_detail.'</div>';
			}else{
			...
			if($temp->jawaban_benar==1){
            	$record[] = '<button type="button" class="btn btn-success btn-xs">Benar</button>';
            }else{
            	$record[] = '<button type="button" class="btn btn-warning btn-xs">Salah</button>';
            }
			
            $record[] = '<div class="btn-group btn-group-sm" role="group" style="text-align: center;">
            	<a class="btn btn-warning" onclick="edit(\''.$temp->jawaban_id.'\')" title="Edit Jawaban" style="cursor: pointer;"><span class="glyphicon glyphicon-pencil"></span> </a>
            	<a class="btn btn-danger" onclick="hapus(\''.$temp->jawaban_id.'\')" title="Hapus Jawaban" style="cursor: pointer;"><span class="glyphicon glyphicon-trash"></span> </a>
            	</div>
            ';

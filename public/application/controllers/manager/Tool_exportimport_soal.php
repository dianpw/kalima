...
class Tool_exportimport_soal extends Member_Controller {
    ...
    public function exportsoal(){
		...
		$this->zip->add_data('kalima-soal.sql', $sql);

		$path = $this->config->item('upload_path');
		$this->zip->read_dir($path);

		$this->zip->download('kalima-soal.zip');
    }
    ...
    public function exportsoal(){
		...
		$this->zip->add_data('kalima-soal.sql', $backup);
    ...
    public function importsoal(){
        ...
        if(!empty($_FILES['userfile']['name'])){    
	        // Cek apakah masih ada mata pelajaran atau tidak, jika mata pelajaran masih ada, maka proses digagalkan
	        if($this->cbt_topik_model->count_all()->row()->hasil>0){
	        	$data['error'] = 'Data Soal Gagal di Import. Masih ada Mata Pelajaran di KALIMA TEST. Silahkan hapus Mata Pelajaran terlebih dahulu.';
	        }else{
	            ...
	            if (!$this->upload->do_upload()){
			        ...
			        if ($zip->open($config['upload_path'].$upload_data['file_name']) === TRUE)			        {
			            ...
			            // Import SQL
			            if (file_exists($config['upload_path'].'import/kalima-soal.sql')) {
			            	$isi_file = file_get_contents($config['upload_path'].'import/kalima-soal.sql'); 

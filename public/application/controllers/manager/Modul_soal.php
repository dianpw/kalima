...
class Modul_soal extends Member_Controller {
    ...
    public function index($id_soal=null){
        ...
        if($id_soal!=null){
        	...
        	if($query_soal->num_rows()>0){
        		...
        		$data['data_aktif'] = $query_soal->soal_aktif;
        ...
        if($query_user->num_rows()>0){
        	...
        	// Mengecek apakah user dibatasi hanya mengentry beberapa mata pelajaran
        	if(!empty($query_user->opsi1)){
        		...
        		// Jika user tidak dibatasi mengedit soal sesuai mata pelajaran
        ...
        if($counter==0){
        	$select = '<option value="kosong">Tidak Ada Data Mata Pelajaran</option>';
        }
    ...
    function tambah(){
        ...
        $this->form_validation->set_rules('tambah-topik-id', 'Mata Pelajaran','required');
        ...
        if($this->form_validation->run() == TRUE){
        	...
        	$posisi = $this->config->item('upload_path').'/mapel_'.$id_topik.'';
        	...
        	if($id_topik=='kosong'){
        		$status['status'] = 0;
            	$status['pesan'] = 'Mata Pelajaran belum tersedia';
        ...
        if($this->form_validation->run() == TRUE){
        	...
        	if($query_soal->num_rows()>0){
        		...
        		if($this->cbt_tes_topik_set_model->count_by_kolom('tset_topik_id', $query_soal->soal_topik_id)->row()->hasil>0){
	        		$status['status'] = 0;
	            	$status['pesan'] = 'Soal tidak bisa dihapus, Mata Pelajaran soal masih dipakai pada Tes.';
	        	}else{
    ...
    function upload_file(){
        ...
        $this->form_validation->set_rules('image-topik-id', 'Mata Pelajaran','required');
    ...
    function get_datatable(){
	    ...
	    foreach ($query as $temp) {			
			...
			if(!empty($temp->soal_audio)){
				$posisi = $this->config->item('upload_path').'/mapel_'.$temp->soal_topik_id;
				$soal = $soal.'<br />
					<audio controls>
            ...
            if($temp->soal_tipe!=2 AND $temp->soal_tipe!=3){
            	$record[] = '<div class="btn-group btn-group-sm" role="group" style="text-align: center;">
	            	<a class="btn btn-primary" onclick="jawaban(\''.$temp->soal_id.'\')" title="Tambah Jawaban" style="cursor: pointer;"><span class="glyphicon glyphicon glyphicon-plus"></span> Jawaban</a>
	            	<a class="btn btn-warning" onclick="edit(\''.$temp->soal_id.'\')" title="Edit Soal" style="cursor: pointer;"><span class="glyphicon glyphicon-pencil"></span> Soal</a>
	            	<a class="btn btn-danger" onclick="hapus(\''.$temp->soal_id.'\')" title="Hapus Soal" style="cursor: pointer;"><span class="glyphicon glyphicon-trash"></span> Soal</a>
	            	</div>
	            ';
            }else{
            	$record[] = '<div class="btn-group btn-group-sm" role="group" style="text-align: center;">
	...
	function get_datatable_image(){
		$topik = $this->input->get('topik');
		if(!empty($topik)){
			$posisi = $this->config->item('upload_path').'/mapel_'.$topik;
		}else{
	    ...
	    foreach ($query as $temp) {			
			...
			if(!is_dir($posisi.'/'.$temp)){
            	if($info['extension']=='jpg' or $info['extension']=='png' or $info['extension']=='jpeg' or $info['extension']=='gif'){
            		...
            		$record[] = '<a onclick="image_preview(\''.$posisi.'\',\''.$temp.'\')" style="cursor: pointer;" class="btn btn-primary btn-xs">Pilih</a>';
            		$output['aaData'][] = $record;

...
class Tes_tambah extends Member_Controller {
    ...
    function get_topik_by_modul($modul=null){
        ...
        $data['select_topik'] = '<option value="kosong">Tidak Ada Mata Pelajaran</option>';
        if(!empty($modul)){
            $data['data']=1;
    ...
    function tambah_soal(){
        ...
        $this->form_validation->set_rules('soal-topik', 'Mata Pelajaran','required|strip_tags');
        ...
        if($this->form_validation->run() == TRUE){
            ...
            if($this->cbt_tes_user_model->count_by_kolom('tesuser_tes_id', $data['tset_tes_id'])->row()->hasil>0){
                ...
                // Apakah jml soal yang dimasukkan sudah sesuai dengan jumlah soal pada mata pelajaran
                if($data['tset_jumlah']<=$jml_soal){
                    ...
                    $status['status'] = 0;
                    $status['pesan'] = 'Jumlah soal pada Mata Pelajaran lebih sedikit dari Jumlah Soal yang diminta';
                }
            }   
        }else{
        	$status['status'] = 0;

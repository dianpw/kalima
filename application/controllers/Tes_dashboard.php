...
class Tes_dashboard extends Tes_Controller {
				...
				if($this->cbt_tes_user_model->count_by_user_tes($user_id, $tes_id)->row()->hasil==0){
					...
					if($is_ok==1){
						...
						if($this->cbt_tes_topik_set_model->count_by_kolom('tset_tes_id', $query_tes->tes_id)->row()->hasil>0){
							...
							// Mengambil data mata pelajaran yang ada pada tes
							$query_subject_set = $this->cbt_tes_topik_set_model->get_by_kolom('tset_tes_id', $tes_id)->result();
							$i_soal = 0;
							// Mengambil data mata pelajaran berdasarkan tes
							foreach ($query_subject_set as $subject_set) {
								// Mengambil data soal sesuai jumlah berdasarkan mata pelajaran, tipe, dan kesulitan
    ...
    function get_datatable(){
	    ...
	    foreach ($query as $temp) {			
			...
			// Cek apakah tes yang terdaftar pada kelas memiliki soal sesuai mata pelajaran yang ada
	            ...
	            if($this->cbt_tes_user_model->count_by_user_tes($user_id, $temp->tes_id)->row()->hasil>0){
	            	...
	            	if($tanggal<$tanggal_tes AND $query_test_user->tesuser_status!=4){
		            	...
		            	if($temp->tes_results_to_users==1){
							//$nilai = $this->cbt_tes_soal_model->get_nilai($query_test_user->tesuser_id)->row();
		            		//$record[] = "Nilai: " . round($this->cbt_tes_soal_model->get_nilai($query_test_user->tesuser_id)->row()->hasil1 / $temp->tes_max_score * 100);
		            		//$record[] = round(($nilai->hasil1/$temp->tes_max_score * 60)+(($nilai->hasil2+$nilai->hasil3)/$temp->tes_max_score * 40));

...
class Peserta_daftar extends Member_Controller {
    ...
    function tambah(){
        ...
        $this->form_validation->set_rules('tambah-group', 'Kelas','required|strip_tags');
        ...
        if($this->form_validation->run() == TRUE){
            ...
            if($this->cbt_user_grup_model->count_by_kolom('grup_id', $data['user_grup_id'])->row()->hasil>0){
            	...
            	$status['status'] = 0;
                $status['pesan'] = 'Data Kelas tidak tersedia, Silahkan tambah data Kelas';
            }
        }else{
            $status['status'] = 0;
    ...
    function edit(){
        ...
        $this->form_validation->set_rules('edit-group', 'Kelas','required|strip_tags');

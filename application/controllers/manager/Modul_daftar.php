	...
	function cetak_soal($topik_id=null){
		if(!empty($topik_id)){
			...
			if($query_soal->num_rows()>0){
				...
				$soal_table = '
					<H3>Daftar Soal</h3>
					<b>Mata Pelajaran = '.$query_topik->topik_nama.'</b>
					<hr />
	    ...
	    foreach ($query as $temp) {			
            ...
            $jawaban_table = '
            	<table class="table" border="0">
            		<tr>
                      ...
                      <td width="15%"><a class="btn btn-warning" href="'.site_url().'/manager/modul_soal/index/'.$temp->soal_id.'" title="Edit Soal" style="cursor: pointer;"><span class="glyphicon glyphicon-pencil"></span> Soal</a></td>
                    </tr>
            ';
            ...
            if($temp->soal_tipe==1){
	            ...
	            if($query_jawaban->num_rows()>0){
	            	...
	            	foreach ($query_jawaban as $jawaban) {						
						if($jawaban->jawaban_benar==1){
							$temp_benar = '<div class="alert alert-success d-flex align-items-center" role="alert" style="padding: 5px;margin-bottom: 0;"> Benar</div>';
							$temp_jawaban = '<div class="alert alert-success d-flex align-items-center" role="alert" style="padding: 5px;margin-bottom: 0;"><span class="glyphicon glyphicon-ok-circle"></span> '.$jawaban->jawaban_detail.'</div>';
	            		...
	            		$jawaban_table = $jawaban_table.'
	            			<tr>
		                      	...
		                      	<td width="5%">'.$temp_benar.'</td>
		                      	<td width="75%">'.$temp_jawaban.'</td>
		                      	<td width="15%"><a class="btn btn-primary" href="'.site_url().'/manager/modul_jawaban/index/'.$temp->soal_id.'/'.$jawaban->jawaban_id.'" title="Edit Jawaban" style="cursor: pointer;"><span class="glyphicon glyphicon-pencil"></span> Jawaban</a></td>
		                    </tr>
	            		';
	            	}
	            }
            	...
            	$jawaban_table = $jawaban_table.'
	            			<tr>
		                      	...
		                      	<td colspan="2"><div class="alert alert-success" role="alert" style="padding: 5px;margin-bottom: 0;">'.$temp->soal_kunci.'</div></td>
		                    </tr>
	            		';
            }
